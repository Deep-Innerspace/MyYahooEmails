...
def stats_overview():
    ...
    with get_db() as conn:
        ...
        received = conn.execute("SELECT COUNT(*) FROM emails WHERE direction='received'").fetchone()[0]
        threads = conn.execute("SELECT COUNT(*) FROM threads").fetchone()[0]
        french = conn.execute("SELECT COUNT(*) FROM emails WHERE language='fr'").fetchone()[0]
        english = conn.execute("SELECT COUNT(*) FROM emails WHERE language='en'").fetchone()[0]
        with_attachments = conn.execute("SELECT COUNT(*) FROM emails WHERE has_attachments=1").fetchone()[0]
        first_date = conn.execute("SELECT MIN(date) FROM emails").fetchone()[0]
        last_date = conn.execute("SELECT MAX(date) FROM emails").fetchone()[0]
    ...
    table.add_row("", "")
    table.add_row("Language: French", str(french))
    table.add_row("Language: English", str(english))
    table.add_row("", "")
    table.add_row("Date range", f"{first_date[:10] if first_date else 'N/A'} → {last_date[:10] if last_date else 'N/A'}")
    table.add_row("Topics defined", str(topics_count))
    table.add_row("Analysis runs", str(runs_count))
...
def stats_frequency(by, contact):
    ...
    with get_db() as conn:
        ...
        where = ""
        if contact:
            where = "WHERE (from_address=? OR to_addresses LIKE ?)"
...
def analyze_classify(provider, batch_size, since, limit, force):
...
@click.option("--type", "analysis_type", default=None,
              type=click.Choice(["classify", "tone", "timeline"]),
